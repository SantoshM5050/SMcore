...
try {
  ...
  execSync(`npx -y prisma@5.22.0 generate --schema="${schemaPath}"`, { stdio: 'inherit' });

  if (process.env.DATABASE_URL) {
    console.log('[DB Sync] Pushing Prisma schema changes to active database...');
    execSync(`npx -y prisma@5.22.0 db push --skip-generate --schema="${schemaPath}"`, { stdio: 'inherit' });
  }
